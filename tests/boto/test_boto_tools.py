...
from resnap.boto.tools import format_remote_path_folder_to_search, remove_separator_at_begin, sort_folders_and_files
...
def test_should_sort_folders_and_files(path: str, iterator: list, expected_result: list[str]) -> None:
    # When
    result = sort_folders_and_files(path, iterator)
...
def test_should_raise_if_not_contents() -> None:
    ...
    with pytest.raises(FileNotFoundError, match=r"The directory or object 'test/path/\*' doesn't exist."):
        sort_folders_and_files(path, iterator)
